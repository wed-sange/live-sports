 ...
 */
@Component
public class JimServerAPI3 implements ImConst{
	...
	public static boolean sendToClass(ChatBody chatBody, ImPacket packet) {
		...
		try {
			imChannelContextList.forEach(imChannelContext -> {
				boolean firtSend = true;
				// 发送给目标用户
				if (chatBody.getTo().equals(imChannelContext.getUserId())) {
					send(imChannelContext,packet);
					firtSend = false;
				}
				// 如果是主播组成员才发，不是就不发, 如果发过了就不发了
				if (UserChannelEnum.LIVE.getFlag().equals(imChannelContext.getTioChannelContext().getLoginType())) {
					if (firtSend) {
						send(imChannelContext,packet);
					}
				}
