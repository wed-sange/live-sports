...
public class Tio {
	 ...
	 */
	public static void bindUser(ChannelContext channelContext, String userid, String loginType) {
		channelContext.tioConfig.users.bind(userid, channelContext, loginType);
	}
	...
	private static Boolean sendToAll(TioConfig tioConfig, Packet packet, ChannelContextFilter channelContextFilter, boolean isBlock) {
		try {
			...
			if (setWithLock == null) {
				log.error("{}, sendToAll 当前没有任何连接", tioConfig.getName());
				return false;
			}
	...
	private static Boolean sendToSet(TioConfig tioConfig, SetWithLock<ChannelContext> setWithLock, Packet packet, ChannelContextFilter channelContextFilter, boolean isBlock) {
		...
		try {
			Set<ChannelContext> set = setWithLock.getObj();
			if (set.size() == 0) {
				log.info("{},Set<ChannelContext>  集合为空", tioConfig.getName());
				return false;
			}
