...
public class JimServerAPI implements ImConst {
    ...
    public static Boolean sendToGroup(String groupId, ImPacket packet) {
        ...
        Object o2 = redissonClient.getBucket(CacheConstant.CHAT_LIVE_GROUP_USER_SESSION + CacheConstant.ANCHOR + anchorId).get();
        if (extracted(packet, o2)) {
            return false;
        }
        ...
        RLock rLock = redissonClient.getReadWriteLock(CacheConstant.CHAT_LIVE_GROUP_USER_SESSION).readLock();
    ...
    public static boolean bindUser(ImChannelContext imChannelContext, String userId) {
        ...
        if (Objects.isNull(user)) {
            user = User.builder().userId(userId).status(UserStatusType.ONLINE.getStatus()).build();
        }
    ...
    public static boolean unbindUser(String userId) {
        return unbindUser(User.builder().userId(userId).build());
    }
    ...
    public static boolean isInGroup(String groupId, ImChannelContext imChannelContext) {
        try {
            if (StringUtils.isEmpty(groupId)) {
                log.error("groupId is null");
                return false;
            }
            return Tio.isInGroup(groupId,imChannelContext.getTioChannelContext());
        } catch (Exception e) {
            log.error("an exception occurred in the unBinding group", e);
            return false;
        }
    ...
    private static ImPacket convertPacket(ImChannelContext imChannelContext, ImPacket packet) {
        ...
        try {
            ...
            respPacket.setSendId(packet.getSendId());
            if (respPacket == null) {
    ...
    public static boolean sendToClass(ChatBody chatBody, ImPacket packet, ImChannelContext defaultChannelContext) {
        String liveId = chatBody.getAnchorId();
        // 如果为空 则是用户发送给主播的，那么toId就是LiveId;
        if (StringUtils.isBlank(liveId)) {
            liveId = chatBody.getTo();
        }
//            LiveGroupCache.READ_LOCK.lock();
        RLock rLock = redissonClient.getReadWriteLock(CacheConstant.CHAT_LIVE_GROUP_USER_SESSION).readLock();
        rLock.lock(10, TimeUnit.SECONDS);
        try {
            // 直播人员发送给用户 除发送者自己其他人都要收到(只要不是0的类型都是直播端用户)
            if (!IdentityType.APP_USER.getCode().equals(chatBody.getIdentityType())) {
                // 发给用户
                List<ImChannelContext> userChannels = getByUserId(chatBody.getTo());
                userChannels.forEach(a -> {
                    send(a, packet);
                });
                // 发给所有主播组成员
                Object o = redissonClient.getBucket(CacheConstant.CHAT_LIVE_GROUP_USER_SESSION + CacheConstant.ANCHOR + liveId).get();
                if (ObjectUtil.isNull(o)) {
                    return false;
                }
                List<String> strings = (List<String>) o;
                strings.forEach(a -> {
                    List<ImChannelContext> channelContexts = getByUserId(a);
                    channelContexts.forEach(b -> {
                        send(b, packet);
                    });
                });
            } else {
                ...
                // 发给所有主播组成员
                // 这里先发送给自己，好做多端聊天信息同步
                boolean sendFlag = sendToUser(chatBody.getFrom(), packet);
                if (!sendFlag) {
                    send(defaultChannelContext, packet);
                }
                Object o = redissonClient.getBucket(CacheConstant.CHAT_LIVE_GROUP_USER_SESSION + CacheConstant.ANCHOR + liveId).get();
                if (extracted(packet, o)) {
                    return false;
                }
            }
            ...
            if (Objects.nonNull(cluster) && !packet.isFromCluster()) {
                ...
                strings.forEach(a -> {
                    List<ImChannelContext> channelContexts = getByUserId(a);
                    channelContexts.forEach(b -> cluster.clusterToUser(b.getUserId(), packet));
                });
            }
        }
    }

    private static boolean extracted(ImPacket packet, Object o2) {
        if (ObjectUtil.isNull(o2)) {
            return true;
        }
        List<String> operators = (List<String>) o2;
        operators.forEach(a -> sendToUser(a, packet));
        return false;
    }
}
