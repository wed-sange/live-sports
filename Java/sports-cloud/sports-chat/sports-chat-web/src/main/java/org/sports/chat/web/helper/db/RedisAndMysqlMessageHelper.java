package org.sports.chat.web.helper.db;
...
import org.sports.chat.web.helper.mysql.MysqlImStoreBindListener;
 ...
 */
public class RedisAndMysqlMessageHelper extends AbstractMessageHelper {

    private Logger log = LoggerFactory.getLogger(RedisAndMysqlMessageHelper.class);

    private static final String SUFFIX = ":";

    private IChatService chatService = SpringUtil.getBean(IChatService.class);
    private ChatGroupUserMapper chatGroupUserMapper = SpringUtil.getBean(ChatGroupUserMapper.class);

    public RedisAndMysqlMessageHelper() {
    ...
    public boolean isOnline(String userId) {
        try {
            ...
            while (terminalKeyIterator.hasNext()) {
                ...
                Integer isOnline = RedisCacheManager.getCache(USER).get(terminalKey, Integer.class);
                if (UserStatusType.ONLINE.getNumber() == isOnline) {
                    return true;
                }
            }
        } catch (Exception e) {
    ...
    public void removeGroupUser(String userId, String groupId) {
        ...
        RedisCacheManager.getCache(ImConst.GROUP).listRemove(groupId, userId);
    }
    ...
    public boolean updateUserTerminal(User user) {
        ...
        chatUserDO.setLvName(user.getLvName());
        chatUserDO.setLvNum(user.getLvNum());
        chatUserDO.setIdentity(user.getIdentity());
        if (UserStatusType.ONLINE.getStatus().equals(user.getStatus())) {
            ...
            //如果用户下线，则将用户正在访问该直播间的状态修改为未正在访问。
            ChatGroupUserDO groupUserDO = new ChatGroupUserDO();
            ...
            LambdaQueryWrapper<ChatGroupUserDO> queryWrapper = Wrappers.lambdaQuery(ChatGroupUserDO.class).eq(ChatGroupUserDO::getUserId, userId).eq(ChatGroupUserDO::getStatus, UserStatusType.ONLINE.getNumber());
            chatGroupUserMapper.update(groupUserDO, queryWrapper);
        }
        chatUserDO.setSign(user.getSign());
        chatUserDO.setTerminal(user.getTerminal());
        chatService.saveChatUser(chatUserDO);
        RedisCacheManager.getCache(ImConst.USER).put(chatUserDO.getUserId() + ":" + ImConst.TERMINAL + ":" + chatUserDO.getTerminal(), chatUserDO.getStatus());
        return true;
    }
