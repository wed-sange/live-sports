...
<template>
  <div
    class="message-card-box w-full pl-[22px] pr-[18px] my-[12px] py-[2px] relative"
  >
    <div class="message-card w-full flex items-stretch">
      ...
      >
        ...
        <div class="w-full flex items-center pt-[4px]">
          <div class="message-card__info--message flex-auto truncate">
            当前会话
          </div>
        </div>
      </div>
...
import { type ChatAccountModel } from "./useCommon";
defineOptions({
  name: "ChatHeader"
});
withDefaults(
  defineProps<{
    account: ChatAccountModel;
  }>(),
  {
    account: () => {
      return {} as ChatAccountModel;
    }
  }
);
</script>
