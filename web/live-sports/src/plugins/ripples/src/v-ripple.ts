...
const ripple = (event: PointerEvent, el: HTMLElement, options: IRippleDirectiveOptions): void => {
  ...
  const size = MULTIPLE_NUMBER * getDistanceToFurthestCorner(x as number, y as number, rect);

  const rippleContainer = createContainer(computedStyles);
  const rippleEl = createrippleElement(x as number, y as number, size, options);
