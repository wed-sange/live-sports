...
import { formatMessageTime } from './message';
...
export const buildChatMessage = ({
  ...
  return {
    ...
    accountAvatar: accountAvatar || (isMe ? defaultUserLogo : defaultAnchorLogo),
    isMe,
    level: level || '',
    tip: tip || '',
    status: status,
...
export const useShowDateTip = () => {
  ...
  const getShowDateTip = (createTime?: dayjs.Dayjs) => {
    ...
    if (createTime) {
      ...
      const createByTime = formatMessageTime(createTime.valueOf(), 'now');
      if (!tipCache.has(key)) {
